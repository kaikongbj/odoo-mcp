...
def check_tool_functions(file_path):
    ...
    # 检查是否直接调用 _get_env() (排除 _safe_execute_with_env 方法内部的调用)
    lines = content.split('\n')
    direct_calls = 0
    in_safe_method = False

    for i, line in enumerate(lines):
        # 检查是否在 _safe_execute_with_env 方法内部
        if 'def _safe_execute_with_env(' in line:
            in_safe_method = True
        elif re.match(r'^\s*def ', line) and in_safe_method:
            in_safe_method = False

        # 如果不在安全方法内部且包含直接调用
        if 'env = self._get_env()' in line and not in_safe_method:
            direct_calls += 1
            print(f"第 {i + 1} 行发现直接调用: {line.strip()}")

    if direct_calls > 0:
        print(f"\n⚠️ 发现 {direct_calls} 处直接调用 _get_env() 的情况")
